...
object CardTableMigrations : BaseTableMigration() {
    ...
    private const val TABLE_BACKUP = "${AppDatabase.CARD_TABLE}_BACKUP"
    ...
    private val COPY_TABLE_DATA = copyFieldsFromTableToTable(
        fields = TABLE_FIELDS, originTable = AppDatabase.CARD_TABLE, destinyTable = TABLE_BACKUP
    )
    private val DROP_TABLE = dropTable(AppDatabase.CARD_TABLE)
    private val RENAME_TABLE_BACKUP_TO_TABLE = renameTableTo(
        oldTable = TABLE_BACKUP, newTable = AppDatabase.CARD_TABLE
    )
